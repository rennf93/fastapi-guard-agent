...
from guard_agent.models import SecurityEvent, AgentStatus, AgentConfig, SecurityMetric
...
class TestHTTPTransport:
    ...
    @pytest.mark.asyncio
    async def test_send_events_success(self, agent_config: AgentConfig, mock_session: AsyncMock) -> None:
    ...
    @pytest.mark.asyncio
    async def test_send_events_failure(self, agent_config: AgentConfig, mock_session: AsyncMock) -> None:
    ...
    @pytest.mark.asyncio
    async def test_send_events_exception_during_send(self, agent_config: AgentConfig, mock_session: AsyncMock) -> None:
        """Test send_events when an exception occurs during the send process."""
        transport = HTTPTransport(agent_config)
        transport._session = mock_session
        transport._send_with_retry = AsyncMock(side_effect=Exception("Test Send Events Exception"))
    ...
    @pytest.mark.asyncio
    async def test_send_metrics_exception_during_send(self, agent_config: AgentConfig, mock_session: AsyncMock) -> None:
        """Test send_metrics when an exception occurs during the send process."""
        transport = HTTPTransport(agent_config)
        transport._session = mock_session
        transport._send_with_retry = AsyncMock(side_effect=Exception("Test Send Metrics Exception"))
    ...
    @pytest.mark.asyncio
    async def test_retry_logic(self, agent_config: AgentConfig, mock_session: AsyncMock) -> None:
        ...
        mock_session.post.side_effect = lambda *args, **kwargs: create_context_manager(500 if call_count == 0 else 200, "Server Error" if call_count == 0 else {"status": "ok"})
    ...
    @pytest.mark.asyncio
    async def test_send_with_retry_all_attempts_fail(self, agent_config: AgentConfig, mock_session: AsyncMock) -> None:
    ...
    @pytest.mark.asyncio
    async def test_send_with_retry_make_request_returns_false(self, agent_config: AgentConfig, mock_session: AsyncMock) -> None:
        """Test _send_with_retry when _make_request returns False (e.g., client error)."""
        agent_config.retry_attempts = 0  # No retries
        transport = HTTPTransport(agent_config)
        transport._session = mock_session

        with patch.object(transport, '_make_request', return_value=False) as mock_make_request:
            events = [SecurityEvent(
                timestamp=datetime.now(timezone.utc),
                event_type="ip_banned",
                ip_address="192.168.1.1",
    ...
    @pytest.mark.asyncio
    async def test_get_with_retry_make_request_returns_none(self, agent_config: AgentConfig, mock_session: AsyncMock) -> None:
        """Test _get_with_retry when _make_request returns None (e.g., no rules)."""
        agent_config.retry_attempts = 0  # No retries
        transport = HTTPTransport(agent_config)
        transport._session = mock_session

        with patch.object(transport, '_make_request', return_value=None) as mock_make_request:
    ...
    @pytest.mark.asyncio
    async def test_make_request_session_init_fails(self, agent_config: AgentConfig) -> None:
        """Test _make_request when session initialization fails."""
        transport = HTTPTransport(agent_config)
        transport._session = None

        with patch.object(transport, 'initialize', side_effect=Exception("Init Failed")):
            with pytest.raises(Exception, match="Init Failed"):
                await transport._make_request("POST", "/test", {"key": "value"})

    @pytest.mark.asyncio
    async def test_fetch_dynamic_rules(self, agent_config: AgentConfig, mock_session: AsyncMock) -> None:
    ...
    @pytest.mark.asyncio
    async def test_fetch_dynamic_rules_failure(self, agent_config: AgentConfig, mock_session: AsyncMock) -> None:
    ...
    @pytest.mark.asyncio
    async def test_send_status(self, agent_config: AgentConfig, mock_session: AsyncMock) -> None:
    ...
    @pytest.mark.asyncio
    async def test_close(self, agent_config: AgentConfig, mock_session: AsyncMock) -> None:
    ...
    @pytest.mark.asyncio
    async def test_initialize_already_initialized(self, agent_config: AgentConfig, mock_session: AsyncMock) -> None:
    ...
    @pytest.mark.asyncio
    async def test_get_with_retry_rate_limited(self, agent_config: AgentConfig, mock_session: AsyncMock) -> None:
    ...
    @pytest.mark.asyncio
    async def test_make_request_session_none_after_initialize(self, agent_config: AgentConfig) -> None:
        ...
        with patch.object(transport, 'initialize', wraps=transport.initialize) as mock_initialize:
            with patch('aiohttp.ClientSession.post') as mock_post:
                ...
                result = await transport._make_request("POST", "/test", {"key": "value"})
                assert result == {"status": "ok"}
                mock_initialize.assert_called_once()
                mock_post.assert_called_once()

    @pytest.mark.asyncio
    async def test_send_with_retry_rate_limited(self, agent_config: AgentConfig, mock_session: AsyncMock) -> None:
    ...
    @pytest.mark.asyncio
    async def test_send_with_retry_circuit_breaker_open(self, agent_config: AgentConfig, mock_session: AsyncMock) -> None:
    ...
    @pytest.mark.asyncio
    async def test_make_request_client_error(self, agent_config: AgentConfig, mock_session: AsyncMock) -> None:
    ...
    @pytest.mark.asyncio
    async def test_make_request_timeout_error(self, agent_config: AgentConfig, mock_session: AsyncMock) -> None:
    ...
    @pytest.mark.asyncio
    async def test_make_request_generic_exception(self, agent_config: AgentConfig, mock_session: AsyncMock) -> None:
    ...
    @pytest.mark.asyncio
    async def test_make_request_unsupported_method(self, agent_config: AgentConfig) -> None:
    ...
    @pytest.mark.asyncio
    async def test_handle_response_200_non_json(self, agent_config: AgentConfig) -> None:
    ...
    @pytest.mark.asyncio
    async def test_handle_response_429_rate_limited(self, agent_config: AgentConfig) -> None:
    ...
    @pytest.mark.asyncio
    async def test_handle_response_401_unauthorized(self, agent_config: AgentConfig) -> None:
    ...
    @pytest.mark.asyncio
    async def test_handle_response_403_forbidden(self, agent_config: AgentConfig) -> None:
    ...
    @pytest.mark.asyncio
    async def test_handle_response_500_server_error(self, agent_config: AgentConfig) -> None:
    ...
    @pytest.mark.asyncio
    async def test_handle_response_other_client_error(self, agent_config: AgentConfig) -> None:
    ...
    @pytest.mark.asyncio
    async def test_close_connector_closed(self, agent_config: AgentConfig, mock_session: AsyncMock) -> None:
    ...
    @pytest.mark.asyncio
    async def test_get_with_retry_rate_limited_fetch_rules(self, agent_config: AgentConfig, mock_session: AsyncMock) -> None:
        ...
        with patch.object(transport.rate_limiter, 'acquire', side_effect=[False, True]) as mock_acquire:
            with patch.object(transport.rate_limiter, 'get_retry_after', return_value=0.1) as mock_get_retry_after:
                with patch('asyncio.sleep') as mock_sleep:
                    ...
                    assert mock_acquire.call_count == 2  # Called twice due to rate limiting
                    mock_get_retry_after.assert_called_once()  # Should get retry delay
                    mock_sleep.assert_called_once_with(0.1)  # Should sleep for retry delay
    ...
    @pytest.mark.asyncio
    async def test_make_request_session_remains_none_after_initialize(self, agent_config: AgentConfig) -> None:
    ...
    @pytest.mark.asyncio
    async def test_fetch_dynamic_rules_invalid_response_data(self, agent_config: AgentConfig, mock_session: AsyncMock) -> None:
        """Test fetch_dynamic_rules when response data is invalid and causes DynamicRules parsing to fail."""
        transport = HTTPTransport(agent_config)
        transport._session = mock_session

        # Mock _get_with_retry to return invalid data that will cause DynamicRules(**response_data) to fail
        ...
        with patch.object(transport, '_get_with_retry', return_value=invalid_response_data):
            rules = await transport.fetch_dynamic_rules()
    ...
    @pytest.mark.asyncio
    async def test_transport_interface_compatibility(self, mock_transport: AsyncMock) -> None:
